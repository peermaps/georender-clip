...
function cutMeridian(m) {
  ...
  /*
  if (geo.pointInPolygon(northPole, m)) {
    console.error('NORTH',m)
    var ms = []
    for (var i = 0; i < m.length; i++) {
      var m0 = m[i], m1 = m[(i+1)%m.length]
      var apex = gcApex([],m0,m1)
      console.log('apex',
        apex[0].toFixed(2) + ',' + apex[1].toFixed(2),
        m0[0].toFixed(2) + ',' + m0[1].toFixed(2),
        m1[0].toFixed(2) + ',' + m1[1].toFixed(2)
      )
      //ms.push([[m[i],m[(i+1)%m.length],northPole]])
    }
    //return ms
  }
  if (geo.pointInPolygon(southPole, m)) {
    console.error('SOUTH',m)
  }
  */
  calcBbox(bbox, m)
